...
function steps() {
  ...
  return [
    ...
    {
      id: 'install',
      done: !!s.settings.installed || matchMedia('(display-mode: standalone)').matches,
      title: 'להתקין על מסך הבית',
      why: 'בלי אייקון בטלפון אין "רעיון מהיר" ואין לאן לשתף — והרעיון באמצע הפודקאסט ילך שוב לוואטסאפ לעצמך.',
      action: ['התקן', async () => {
        const A = await import('../app.js');
        if (A.isInstalled()) { toast('כבר מותקן', 'ok'); update(st => { st.settings.installed = true; }); refresh(); return; }
        const r = await A.promptInstall();
        if (r === 'unavailable') toast('בכרום: תפריט ⋮ → "התקן אפליקציה". באייפון: שיתוף → "הוסף למסך הבית".', 'err');
        else if (r === 'accepted') { toast('הותקן', 'ok'); refresh(); }
      }]
    },
    {
      id: 'sync',
      done: !!s.settings.syncEnabled && !!localStorage.getItem('front.syncToken'),
      title: 'להפעיל סנכרון',
      why: 'בלי זה מה שקלטת בטלפון לא מגיע למחשב. עם זה זה קורה לבד, כולל מחיקות.',
      action: ['להגדרות', () => go('#/settings')]
    },
    {
      id: 'backup',
...
function render(root) {
  ...
  /* --- הסיור. ראשון בעמוד, כי זה מה שעונה על "לא הבנתי מה זה" --- */
  const tourCard = el('div', { class: 'card tourcard' });
  tourCard.append(el('div', { class: 'tourcard-t' },
    el('div', { style: { flex: 1, minWidth: 0 } },
      el('h3', {}, '👋 קח אותי סיבוב במערכת'),
      el('div', { class: 'small muted', style: { marginTop: '5px', lineHeight: '1.7' } },
        'המערכת תדליק אור על כפתור אחד בכל פעם ותסביר במשפט מה הוא עושה. ' +
        'אפשר לעצור באמצע, ואפשר לחזור לזה מתי שתרצה.'))));
  const tRow = el('div', { class: 'row', style: { marginTop: '12px', flexWrap: 'wrap' } });
  TOUR.list().forEach((t, i) => tRow.append(el('button', {
    class: 'btn btn-sm ' + (i === 0 ? 'btn-y' : ''),
    onclick: () => TOUR.start(t.id)
  }, (i === 0 ? '▶ ' : '') + t.name + ' · ' + t.sub)));
  tourCard.append(tRow);
  root.append(tourCard);
  ...
  root.append(el('div', { class: 'grid g3' },
    ...
    routineCard('⚡', 'במהלך היום', 'לחיצה פה ושם', [
      ...
      ['בטלפון — ⚡ רעיון מהיר', 'לחיצה ארוכה על האייקון → "הכתב". מסך אחד, מדבר, שומר.'],
...
const PAGES_INFO = [
  ...
  ['🎞', 'בנק', 'שוטים ותבניות שכבר עבדו, עם הפרומפט שיצר אותם.', '#/bank'],
  ...
  ['⚙︎', 'הגדרות', 'גיבוי, הרשאות, מחירים, עבודה על העסק.', '#/settings']
];
...
const FAQ = [
  ...
  ['שכחתי להחליף טיימר וזה רץ על הדבר הלא נכון',
    'הכי מהר: "עוד ▾" בסרגל הטיימר → "העבר זמן אחרון לפרויקט אחר" — בוחר כמה דקות אחורה ולאן, ' +
    'והמערכת מפצלת את הרשומות לבד בלי לאבד דקה. ' +
    'שכחת דווקא להתחיל? באותו תפריט יש "התחלתי לפני 15 דק\'". ' +
    'ולתיקונים עדינים — עמוד זמן → ציר היום, גוררים את הקצוות. כפתור 🧹 סורק 30 יום ומציע תיקונים בלחיצה.'],
  ['קלטתי בטלפון — איך זה מגיע למחשב?',
    'אם הסנכרון פעיל — לבד, תוך שניות. כל מכשיר דוחף את המצב שלו למקום אחד בענן וקורא ' +
    'ממנו את של האחרים: בפתיחה, בכל חזרה ללשונית, וכל חמש דקות. גם מחיקות עוברות. ' +
    'להפעלה: הגדרות → סנכרון → צור טוקן, להדביק אותו ב-SYNC_TOKEN במשתני הסביבה של האתר, ' +
    'ולהדליק בשני המכשירים. "בדוק חיבור" יגיד לך בדיוק מה תקין. ' +
    'בלי סנכרון זה נשאר ידני: ייצוא בטלפון, ובמחשב ייבוא → "צרף למה שיש".'],
  ['יש לי רעיון באמצע פודקאסט ואני בטלפון',
    'לחיצה ארוכה על אייקון פרונט במסך הבית → "הכתב". נפתח מסך אחד עם מיקרופון דולק: ' +
    'אתה מדבר, לוחץ שמור, וזהו. אין ניווט ואין החלטות, והמערכת מסווגת לבד לרעיון / משימה / ידע. ' +
    'ראית משהו באפליקציה אחרת? "שתף" → פרונט, וזה נוחת באותו מסך מוכן. ' +
    'הטקסט נשמר בכל הקלדה, אז גם אם הדפדפן נסגר באמצע — הוא שם.'],
  ['עברתי לפרויקט אחר והמספר התאפס',
    'הוא לא. המספר הגדול בסרגל הוא סך הזמן נטו שנצבר היום על הפריט, לא הרצף הנוכחי — ' +
    'חזרה ליוסי ממשיכה מ-1:47 ולא מאפס. הרצף מופיע קטן לידו. ' +
    'ואם אתה עוצר לגמרי — "⏸ השהה" זוכר על מה עבדת וממשיך בלחיצה, בניגוד ל"עצור" ששוכח.'],
  ['המספרים בעמוד כסף נראים מוזרים',
    'תסתכל על השורה מתחת למספר — היא אומרת מאיפה הוא בא. ' +
    '"הערכה שהקלדת" זה ניחוש, "לפי הטיימר" זה בערך, "נמדד מבדיקות" זה אמין. ' +
  ...
  ['איפה שמים דברים שהם לא לקוח ולא משימה?',
    ...
    '(מודעות, ניירת) — אלה "עבודה על העסק", בהגדרות.'],
