...
export async function open() {
  ...
  win = await documentPictureInPicture.requestWindow({ width: 300, height: 210 });
...
function render() {
  ...
  cands.forEach(c => {
    const on = t && t.itemId === c.id && t.kind !== 'wait';
    const todayMs = T.focusMs(c.id, startOfToday(), Date.now());
    grid.append(e(doc, 'button', {
      class: 'fw-btn' + (on ? ' on' : ''),
      title: c.business || c.title,
      onclick: () => { T.startTimer(c.id); render(); }
    },
      e(doc, 'span', { class: 'fw-btn-t' }, c.title),
  ...
  doc.body.append(e(doc, 'div', { class: 'fw-row' },
    e(doc, 'button', {
      class: 'fw-mini', onclick: () => { T.startFree('learn'); render(); }
    }, '📚 למידה'),
    e(doc, 'button', {
